...
class InstrumentHelper(BaseInstrumentHelper):
    ...
    def get_expirys_dict(self, symbols: Iterable[str]) -> Dict[str, List[str]]:
        ...
        return {
            sym: sorted(exp_list, key=lambda d: datetime.strptime(d, "%d-%b-%Y"))
            for sym, exp_list in expirys_dict.items()
        }
    ...
    def get_option_strikes(
        self, symbol: str, expiry: str, atm_strike: int, count: int
    ) -> List[Dict[str, str]]:
        ...
        return [
            ...
            if row["Symbol"] == symbol
            and row["Expiry"] == expiry
            and lower <= float(row["StrikePrice"]) <= upper
        ]
    ...
    def get_token_symbol_dict(self, selected_expiry):
        if os.path.exists(TXT_FILE):
            ...
            self.token_symbol_dict = (
                df[
                    (df["OptionType"].isin(["PE", "CE"]))
                    & (df["Symbol"] == "NIFTY")
...
if __name__ == "__main__":
    ...
    strikes = instrument_helper.get_option_strikes(
        "NIFTY", nifty_expirys[0], 19000, 100
    )
